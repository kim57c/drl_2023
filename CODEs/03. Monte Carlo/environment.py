...
class Environment(tk.Tk):
    ...
    def _build_canvas(self):
        ...
        create_button ("에피소드 10번", self.episode_many, 0.37)
    ...
    def episode_many(self):
        [self.agent.episode(False) for _ in range(10)]
    ...
    def start_simul(self):
        ...
        while not is_close([x, y]):
            time.sleep(0.5)
            action_values = self.agent.possible_next_action_values([x, y])
    ...
    def write_value_table(self, value_table):
        ...
        def write_value (key, value ):
            ...
            key = key[1:len(key)-1]
            idx = key.index(',')
            i, j = int(key[0:idx]), int(key[idx+1:len(key)])
            value = self.write_text(i, j, round(value,2), gx=65, gy=85, color='blue' )
            self.values.append(value)
        
        [write_value(key, value) for key, value in value_table.items()]
        
                
    
    def write_text(self, x, y, contents, gx=5, gy=5, color='black' ):
        
        x, y = gx+(UNIT*x), gy+(UNIT*y)
        font = ('Avenir', str(12), 'normal')
        return self.canvas.create_text(x, y, fill=color, text=contents, font=font, anchor="nw")
